...
SCRIPT_CMD(do_rpcall)
{
    ...
    ret = execute_script(script->vnum, script, NULL, NULL, info->room, NULL, NULL, NULL, NULL, ch, obj1, obj2, vch,NULL, NULL, NULL,info->phrase,info->trigger,info->trigger_type,0,0,0,0,0);
    if(info->room) {
...
SCRIPT_CMD(do_rpremove)
{
    ...
    if(!fAll && vnum < 1 && !name[0] && !obj) {
        bug ("RpRemove - Invalid object from vnum %ld.", info->room->vnum);
        return;
    }
...
SCRIPT_CMD(do_rpremspell)
{
    ...
    if( found && target->carried_by != NULL && target->wear_loc != WEAR_NONE) {
        if (target->item_type != ITEM_WAND &&
            ...
            // Check if character has lworn (linked list) or carrying (traditional)
