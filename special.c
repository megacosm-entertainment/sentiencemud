...
bool spec_protector(CHAR_DATA *ch)
{
    ...
    while(( vch = (CHAR_DATA *)iterator_nextdata(&it)))
    {
        ...
        if (vch->fighting != NULL && !IS_IMMORTAL(vch) && !str_cmp(vch->in_room->area->name, "plith") && vch->tot_level > 30 && vch->fighting->tot_level < 30 && IS_NPC(vch->fighting))  /* break it up! */
        {
            ...
            AREA_DATA *gaol_area = NULL;
            WNUM wnum;
            if (resolve_widevnum(11308, NULL, &wnum))
                gaol_area = wnum.pArea;
            if (!gaol_area) gaol_area = get_system_area_fallback();
            char_to_room(victim, get_room_index(gaol_area, 11308));
...
bool spec_patrolman(CHAR_DATA *ch)
{
    ...
    if (((obj = get_eq_char(ch,WEAR_NECK_1)) != NULL && obj->pIndexData == get_reserved_obj_index("obj_patrol_whistle")) ||
        ((obj = get_eq_char(ch,WEAR_NECK_2)) != NULL && obj->pIndexData == get_reserved_obj_index("obj_patrol_whistle"))) {
...
bool dragon( CHAR_DATA *ch, char *spell_name )
{
    ...
    if (skill && skill->spell_fun)
        (*skill->spell_fun)(skill, ch->tot_level, ch, victim, TARGET_CHAR, WEAR_NONE, INVOC_INTERNAL);
    return true;
}
...
bool spec_breath_gas( CHAR_DATA *ch )
{
    ...
    SKILL_DATA *skill = skill_find_uid(sn);
    if (skill && skill->spell_fun)
        (*skill->spell_fun)(skill, ch->tot_level, ch, NULL,TARGET_CHAR, WEAR_NONE, INVOC_INTERNAL);
    return true;
}
