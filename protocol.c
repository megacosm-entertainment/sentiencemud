         ...
         if ( ( pMXPTag = GetMxpTag( "VERSION=", MXPBuffer ) ) != NULL )
         {
            ...
            if ( MatchString( "MUSHCLIENT", pClientName ) )
            {
               ...
               if ( strcmp(pMXPTag, "4.02") >= 0 )
               {
                  ...
                  pProtocol->pVariables[eMSDP_ANSI_COLOURS]->ValueInt = 1;
...
bool_t bTerminate = false, bUseMXP = false, bUseMSP = false, bColourOn = false;
   if (apDescriptor->character && IS_SET(apDescriptor->character->act[0], PLR_COLOUR))
   bColourOn = true;
   else
   bColourOn = false;
      ...
      case (char)TELOPT_TTYPE:
         if ( pProtocol->bTTYPE )
         {
               ...
               strcmp(pProtocol->pVariables[eMSDP_CLIENT_ID]->pValueString, pClientName)) )
            {
               ...
               if ( pStartPos != NULL && (MatchString(pStartPos, "-256colour") || MatchString(pStartPos, "-256color")) )
               {
