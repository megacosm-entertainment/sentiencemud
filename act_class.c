...
/**
 * resolve_class_rewards_to_skill_levels - Populate SKILL_CLASS_LEVEL from rewards
 *
 * Called during boot after all classes and skills are loaded. Iterates every
 * class's reward list and creates/updates SKILL_CLASS_LEVEL entries on the
 * corresponding SKILL_DATA for each REWARD_SKILL and REWARD_GROUP.
 *
 * This makes SKILL_CLASS_LEVEL a derived/cached structure populated from
 * class rewards rather than a manually maintained mapping.
 */
void resolve_class_rewards_to_skill_levels(void)
{
    ...
    log_string("resolve_class_rewards_to_skill_levels: Building skill-class mappings from rewards...");
    ...
    for (clazz = class_first(); clazz; clazz = clazz->next) {
        ...
        while ((reward = (CLASS_REWARD *)iterator_nextdata(&it))) {
            if (reward->type == REWARD_SKILL) {
                /* Direct skill reward */
                SKILL_DATA *sd = skill_find(reward->name);
                if (!sd) {
                    log_stringf("  WARNING: Class '%s' reward references unknown skill '%s'",
                                clazz->name, reward->name);
                    continue;
                }

                /* Check if this class already has a SKILL_CLASS_LEVEL for this skill */
                bool found = false;
                if (sd->class_levels) {
                    ITERATOR sclit;
                    SKILL_CLASS_LEVEL *scl;
                    iterator_start(&sclit, sd->class_levels);
                    while ((scl = (SKILL_CLASS_LEVEL *)iterator_nextdata(&sclit))) {
                        if (scl->class_name && !str_cmp(scl->class_name, clazz->name)) {
                            /* Update existing — use earliest level */
                            if (reward->level < scl->level)
                                scl->level = reward->level;
                            if (reward->value > 0)
                                scl->rating = reward->value;
                            found = true;
                            break;
                        }
                    }
                ...
                if (!found) {
                    SKILL_CLASS_LEVEL *scl = alloc_perm(sizeof(SKILL_CLASS_LEVEL));
                    scl->class_name = str_dup(clazz->name);
                    scl->clazz = clazz;
                    scl->level = reward->level;
                    scl->rating = (reward->value > 0) ? reward->value : 1;

                    if (!sd->class_levels)
                        sd->class_levels = list_create(false);
                    list_appendlink(sd->class_levels, scl);
                    levels_created++;
                }

            } else if (reward->type == REWARD_GROUP) {
                /* Group reward — expand group and add each skill */
                SKILL_GROUP *group = skill_group_find(reward->name);
                if (!group) {
                    log_stringf("  WARNING: Class '%s' reward references unknown group '%s'",
                                clazz->name, reward->name);
                    continue;
                }
                ...
                iterator_start(&git, group->contents);
                while ((skill_name = (char *)iterator_nextdata(&git))) {
                    SKILL_DATA *sd = skill_find(skill_name);
                    if (!sd)
                        continue;
    ...
    log_stringf("resolve_class_rewards_to_skill_levels: Processed %d classes, created %d skill-class entries.",
                classes_processed, levels_created);
}
