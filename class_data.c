...
/***************************************************************************
 * Skill Class Level Resolution                                            *
 ***************************************************************************/

/**
 * resolve_skill_class_pointers - Resolve SKILL_CLASS_LEVEL.class_name to CLASS_DATA
 *
 * Called after both skills and classes are loaded. Iterates all SKILL_DATA
 * entries and resolves string-based class_name references in their
 * SKILL_CLASS_LEVEL lists to CLASS_DATA pointers for fast runtime lookups.
 */
void resolve_skill_class_pointers(void)
{
    ...
    for (sd = skill_first(); sd; sd = sd->next) {
        if (!sd->class_levels)
            continue;
        ...
        while ((scl = (SKILL_CLASS_LEVEL *)iterator_nextdata(&it))) {
            if (scl->clazz)
                continue;  /* Already resolved (e.g., from resolve_class_rewards) */

            if (scl->class_name && scl->class_name[0]) {
                scl->clazz = class_find_exact(scl->class_name);
                if (scl->clazz) {
                    resolved++;
                } else {
                    log_stringf("resolve_skill_class_pointers: "
                                "Unknown class '%s' for skill '%s'",
                                scl->class_name, sd->name);
                    unresolved++;
                }
            }
        }
    ...
    log_stringf("resolve_skill_class_pointers: Resolved %d, unresolved %d",
                resolved, unresolved);
}
