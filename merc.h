...
#define MAX_CHURCH_LOG_ENTRIES 2000
...
#define IS_OBJCASTER(ch) (IS_SET((ch)->act[0], ACT_IS_NPC) && (ch)->pIndexData->vnum == MOB_VNUM_OBJCASTER)
...
#define IS_BUILDER(ch, Area) (!IS_NPC(ch) && !IS_SWITCHED(ch) && \
                              (ch->pcdata->security >= Area->security || strstr(Area->builders, ch->name) || strstr(Area->builders, "All")))
 ...
 * Description macros.
 */
#define HANDLE(ch) ((IS_NPC((ch)) || IS_SWITCHED(ch) || IS_MORPHED(ch)) ? (ch)->short_descr : (ch)->name)
