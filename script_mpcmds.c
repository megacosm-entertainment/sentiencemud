...
SCRIPT_CMD(do_mpassist)
{
	...
	multi_hit(info->mob, victim->fighting, NULL, TYPE_UNDEFINED);
}
...
SCRIPT_CMD(do_mpcall)
{
	...
	if (!wnum.pArea || wnum.vnum < 1 || !(script = get_script_index(wnum.pArea, wnum.vnum, PRG_MPROG))) {
	...
	ret = execute_script(script, info->mob, NULL, NULL, NULL, NULL, NULL, NULL, ch, obj1, obj2, vch, NULL,NULL, NULL,info->phrase,info->trigger,info->trigger_type,0,0,0,0,0);
	if(info->mob) {
...
SCRIPT_CMD(do_mpchargemoney)
{
	...
	if (!str_cmp(buf,"haggle")) {
		roll = number_percent();
		if (roll < get_skill(victim, gsk_haggle)) {
			amt -= amt * roll / 200;
			check_improve(victim, gsk_haggle, true, 4);
		}
	}
...
SCRIPT_CMD(do_mpkill)
{
	...
	multi_hit(info->mob, victim, NULL, TYPE_UNDEFINED);
	return;
}
...
SCRIPT_CMD(do_mplink)
{
	...
	case ENT_EXIT:
		...
		if (ex && ex->u1.to_room)
		{
			wnum.pArea = ex->u1.to_room->area;
			wnum.vnum = ex->u1.to_room->vnum;
		}
		break;
	case ENT_MOBILE:
...
SCRIPT_CMD(do_mpmload)
{
	...
	case ENT_MOBILE: if (arg->d.mob) { wnum.pArea = arg->d.mob->pIndexData->area; wnum.vnum = arg->d.mob->pIndexData->vnum; } break;
	default: break;
	}
...
//	else if(!str_cmp(field,"exitdir"))	{ ptr = (int*)&mob->exit_dir; allowpc = true; }
	...
	if (check_catalyst)
	{
		...
		{
			bug("MpAlterMob - Invalid catalyst type selected for catalystusage.",0);
			return;
		}
	...
	case '+':
		if( !allowarith ) {
			bug("MpAlterMob - altermob called with arithmetic operator on a non-arithmetic field.", 0);
			return;
		}
...
SCRIPT_CMD(do_mpinput)
{
	...
	if(!wnum.pArea || wnum.vnum < 1 || !get_script_index(wnum.pArea, wnum.vnum, PRG_MPROG)) return;
...
SCRIPT_CMD(do_mpusecatalyst)
{
	...
	info->mob->progs->lastreturn = use_catalyst(mob,room,type,method,amount,(bool)show);
}
/*
// do_mphunt
...
SCRIPT_CMD(do_mpalterexit)
{
	...
	if (check_material)
	{
		...
		if(!IS_VALID(material)) {
			scriptcmd_bug(info, "MpAlterExit - Invalid material.");
			free_buf(buffer);
			return;
		}
...
SCRIPT_CMD(do_mpcloneroom)
{
	...
	source = get_room_index(wnum.pArea, wnum.vnum);
	if(!source) return;
...
SCRIPT_CMD(do_mpscriptwait)
{
	...
	if(!failure.pArea || failure.vnum < 1 || !get_script_index(failure.pArea, failure.vnum, prog_type)) return;
