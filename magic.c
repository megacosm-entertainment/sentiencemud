	...
	*t = target;
	...
	case TAR_CHAR_OFFENSIVE:
		...
		if (is_safe(ch, victim, true) ||
			...
			ch != victim && !IS_SET(ch->in_room->roomflag[1], ROOM_MULTIPLAY))) {
	...
	case TAR_CHAR_DEFENSIVE:
		if (arg[0]) {
			...
			if (victim != ch && victim->fighting && victim->fighting != ch &&
				...
				!IS_NPC(victim->fighting) && !is_pk(ch) && !IS_SET(ch->in_room->roomflag[0], ROOM_ARENA)) {
	...
	case TAR_OBJ_CHAR_OFF:
		...
		if (target == TARGET_CHAR && (is_safe(ch, victim, true) ||
			...
			!IS_SET(ch->in_room->roomflag[1], ROOM_MULTIPLAY)))) {
			send_to_char("Not on that target.\n\r", ch);
			return false;
		}
		break;
...
void do_cast(CHAR_DATA *ch, char *argument)
{
	...
	if (IS_SET(ch->in_room->roomflag[0], ROOM_NOMAGIC)) {
	...
	if ( IS_VALID(spell->token) ) {
		...
		if(!IS_SET(ch->cast_token->pIndexData->flags,TOKEN_NOSKILLTEST)) {
			chance = 0;
			if (IS_SET(ch->in_room->roomflag[1], ROOM_HARD_MAGIC)) chance += 2;
	...
	if(!IS_NPC(ch)) {
		...
		i = ch->tempstore[0];
		if(IS_SET(ch->in_room->roomflag[1],ROOM_SLOW_MAGIC)) i += number_range(500,1000);
	...
	for (trap = ch->in_room->contents; trap; trap = trap->next_content) {
		if (trap->item_type == ITEM_SPELL_TRAP) {
			...
			if (trap->level <= 0) {
				...
				if (!IS_SET(ch->in_room->roomflag[0], ROOM_SAFE) && !IN_CHAT(ch)) {
...
bool can_escape(CHAR_DATA *ch)
{
	...
	if (!ch->in_room->area->open ||
		...
		IS_SET(ch->in_room->roomflag[0], ROOM_NO_RECALL) ||
