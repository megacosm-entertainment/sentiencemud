  ...
  "classes": [
    {
      ...
      "type": "mage",
      "is_primary": true,
      "flags": ["combative", "caster"],
      "primary_stat": "intelligence",
      "hp_range": [6, 8],
      "gains_mana": true,
      "description": "Masters of the arcane arts.",
      "xp": 185200,
      "active_title": "default",
      "available_titles": [
        {"keyword": "default", "display": "Wizard", "is_default": true},
        {"keyword": "archmage", "display": "Archmage", "is_default": false}
      ],
      "action": null
    },
    {
      ...
      "hp_range": [10, 14],
      "gains_mana": false,
      "description": "Skilled fighters and weapon masters.",
      "xp": 120500,
      "active_title": null,
      "available_titles": [
        {"keyword": "default", "display": "Warrior", "is_default": true}
      ],
    ...
    },
    {
      ...
      "hp_range": [5, 7],
      "gains_mana": true,
      "description": "Gatherers of herbs and natural remedies.",
      "xp": 8400,
      "active_title": null,
      "available_titles": [
        {"keyword": "default", "display": "Herbalist", "is_default": true}
      ],
...
- `available_titles` — array of all title objects defined for this class
  (iterate `clazz->titles` linked list). All non-hidden titles are included;
  filtering by unlock conditions is deferred to a future version.
...
- Expand `sentience_identity_input_t` struct to include `max_level`, `type`,
  `flags`, `primary_stat`, `hp_range`, `gains_mana`, `description`, `xp`,
  `active_title`, `available_titles`, and `action_cmd` fields per class entry,
  plus a traits array and a race_info sub-struct.
...
- After any reputation mutation, invalidate the reputations cache by setting
  `reputation_count = -1` to force a rebuild on next pulse. Mutation sites:
  `gain_reputation()`, `set_reputation_rank()`, `group_gain_reputation()`
  (all in `reputation.c`).
...
- Note: `channel_history_append()` is `static` in `channel_service.c`. The
  callers (`channel_gmcp_broadcast` etc.) are in `channel_gmcp.c` within the
  same `channels/` module, so the `report_id` must be passed via the existing
  `out_report_id` parameter of the append call, then forwarded to the GMCP
  sender functions.
